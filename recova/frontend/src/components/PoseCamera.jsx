...
// stateLabel and angleLabel are exercise-specific display strings
...
// Map exercise-specific display labels back to STATE keys for color lookup
...
// ─────────────────────────────────────────────────────────────────────────────
export default function PoseCamera({ patient, sessionId, onRep, exerciseType = 'squat' }) {
  ...
  // Peak angle tracking: squat/bicep_curl track min (Infinity init), shoulder tracks max (-Infinity)
  const minAngleThisRepRef = useRef(Infinity)   // squat, bicep_curl
  const maxAngleThisRepRef = useRef(-Infinity)  // shoulder_flexion

  // Feature 2: ghost frame patience
  const badFrameCountRef = useRef(0)

  // Feature 4: eccentric velocity tracking (shared across exercises)
  const lastTimeRef  = useRef(null)
  const lastAngleRef = useRef(null)

  // Features 4 & 5 / exercise flags accumulated per rep
  const currentRepFlagsRef = useRef([])

  // Bicep curl: torso angle at CURLING entry — detects swinging torso
  const torsoAtCurlStartRef = useRef(null)

  // Audio: dedup warnings so we only speak once per distinct active warning
  ...
  useEffect(() => {
    ...
    pose.onResults((results) => {
      ...
      // ── Feature 1: Dynamic side selection (exercise-aware) ────────────
      ...
      // Active joint landmarks for this frame
      const joints       = useLeft ? cfg.leftJoints : cfg.rightJoints
      const j1           = lm[joints[0]]   // proximal  (hip / hip / shoulder)
      const j2           = lm[joints[1]]   // vertex    (knee / shoulder / elbow)
      const j3           = lm[joints[2]]   // distal    (ankle / elbow / wrist)
      ...
      // Squat-specific danger: knee < 60° is dangerously deep
      ...
      if (exType === 'squat') {
        // Torso angle for squat: shoulder–hip–knee (same side as active leg)
        ...
        if (state === STATE.IDLE && smoothAngle < 160) {
          ...
          if (!repFiredRef.current) {
            ...
            const baseScore  = depthScore + alignScore + 20
            const formScore  = Math.max(0, Math.min(100, Math.round(baseScore - allFlags.length * 10)))
        ...
        // mainAngle = getAngle(hip, shoulder, elbow) — rises as arm lifts

        if (state === STATE.IDLE && smoothAngle >= 40) {
          repStateRef.current = STATE.DESCENDING   // LIFTING phase
          ...
          maxAngleThisRepRef.current = Math.max(maxAngleThisRepRef.current, mainAngle)
          if (smoothAngle < 140) {
            repStateRef.current = STATE.ASCENDING   // LOWERING phase
            speak(cfg.audio.return)
          }

        } else if (state === STATE.ASCENDING) {
          // Feature 4 (adapted): flag if arm drops too fast during lowering
          if (lastTimeRef.current !== null && lastAngleRef.current !== null) {
            const dt     = (now - lastTimeRef.current) / 1000
            const dAngle = lastAngleRef.current - mainAngle  // positive = angle decreasing = dropping
        ...
        // mainAngle = getAngle(shoulder, elbow, wrist) — decreases as arm curls
        // Torso angle for swinging detection: shoulder–hip–knee (same side)
        const hipIdx    = useLeft ? 23 : 24
        const kneeIdx   = useLeft ? 25 : 26
        const torsoAngle = getAngle(j1, lm[hipIdx], lm[kneeIdx])  // shoulder-hip-knee

        if (state === STATE.IDLE && smoothAngle < 140) {
          repStateRef.current = STATE.DESCENDING   // CURLING phase
          minAngleThisRepRef.current = Infinity
          repFiredRef.current = false
          currentRepFlagsRef.current = []
          torsoAtCurlStartRef.current = torsoAngle  // baseline for swinging check
          speak(cfg.audio.start)

        } else if (state === STATE.DESCENDING) {
          minAngleThisRepRef.current = Math.min(minAngleThisRepRef.current, mainAngle)

          // Feature 5 (bicep variant): flag if torso rocks more than 15° from start
      ...
      // Update velocity trackers at end of every successfully processed frame
